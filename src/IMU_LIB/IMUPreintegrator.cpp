#include "IMU_LIB/IMUPreintegrator.h"
...
void IMUPreintegrator::update(const Vector3d& omega, const Vector3d& acc, const double& dt)
{
    ...
    _cov_P_V_Phi = A*_cov_P_V_Phi*A.transpose() +
      Bg*IMU_LIB::IMU::GetGyrMeasCov()*Bg.transpose() +
      Ca*IMU_LIB::IMU::GetAccMeasCov()*Ca.transpose();
...
//        Ca*IMUData::getAccMeasCov()*Ca.transpose();
    ...
    _initP = (I6x6+dt*Ft) * preP * (I6x6+dt*Ft).transpose()
      + dt*dt* Gt*IMU_LIB::IMU::GetAccMeasCov()*Gt.transpose();
}
